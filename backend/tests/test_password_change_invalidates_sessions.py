...
def test_password_change_invalidates_other_sessions(db_session):
    try:
        c, u, sid_current, sid_other = _setup(db_session)
...
def test_password_change_wrong_current_returns_401(db_session):
    try:
        c, u, _, _ = _setup(db_session)
        r = c.post("/api/auth/password", json={
...
def test_password_change_short_password_returns_400(db_session):
    try:
        c, u, _, _ = _setup(db_session)
        r = c.post("/api/auth/password", json={
            "current_password": "old-pw-12chars",
