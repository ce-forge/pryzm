...
interface PrismIndicatorProps {
  /** Visual sizing. `pill` is 56×28, used inside ThinkingPanel; `block`
   *  is 80×40, used by the standalone ProcessingAnimation. */
  size?: "pill" | "block";
}

/**
 * Animated prism + lighthouse beam. The live-activity indicator across
 * the chat surface — used inside ThinkingPanel (reasoning turns) and
 * inline with the themed phrase in ProcessingAnimation (non-reasoning
 * turns).
 *
 * Layered animations on independent clocks so the prism reads as a
 * living object rather than a static-then-flash machine:
 *
 *   5s lighthouse cycle (shootWhite, shootColor, flareAnim, prismSpin,
 *     raysSweep) — white beam in from left, white-to-rainbow burst at
 *     centre, prism snaps through 120°, rainbow rays pan ±4° as they
 *     exit. The main visual moment.
 *
 *   3.4s breathe (prismFloat, prismBreathe, haloPulse) — gentle vertical
 *     float + stroke-opacity pulse on the prism, scale + opacity pulse
 *     on the radial halo behind it. Off-tempo from the 5s cycle so the
 *     two clocks never lock into a repetitive pattern. The prism feels
 *     suspended in light, not waiting for the next beam shot.
 */
  ...
  return (
    ...
    >
      <defs>
        ...
        {/* Soft halo behind the prism. Radial gradient white→sapphire haze
            →transparent, scaled and faded by haloPulse. */}
