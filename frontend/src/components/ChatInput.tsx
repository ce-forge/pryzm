...
export default function ChatInput({
  ...
  // Helper for Upload Pills
  const getUploadPillClasses = (status: string) => {
    if (status === "success") return "bg-emerald-900/20 border-emerald-800 text-emerald-400";
    if (status === "error") return "bg-red-900/20 border-red-800 text-red-400";
    return "bg-[#1e1f20] border-[#333537] text-gray-300";
  };

  return (
    <div className="w-full max-w-3xl relative">
      
      {/* Uploads Queue Display */}
      {uploads.length > 0 && (
        <div className="flex flex-wrap gap-2 mb-3">
            {uploads.map((u) => (
              <div 
                key={u.id} 
                className={`flex items-center gap-2 px-3 py-1.5 rounded-lg text-xs border ${getUploadPillClasses(u.status)}`}
              >
                {u.status === "uploading" ? (
                ...
                {/* Remove File Button */}
                {(u.status === "success" || u.status === "error" || u.status === "pending") && (
                  <button 
                    type="button" 
        ...
        <form onSubmit={handleInference} className="flex flex-col p-2">
            ...
            <div className="flex justify-between items-center px-2 pb-1">
                <div className="flex gap-1 items-center relative" ref={menuWrapperRef}>
                    ...
                    <button 
                      ...
                      className={`p-2 rounded-full transition-colors ${
                        isAutoTesting 
                          ...
                          : 'text-gray-500 hover:text-gray-300 hover:bg-[#333537]'
                      }`} 
                    ...
                    <span className="text-[10px] text-gray-500 ml-2 font-mono select-none">
                      ~{totalTokens} / 8192
                    </span>
                </div>
                ...
                {isProcessing ? (
                    <button 
                      type="button" 
                      onClick={(e) => { e.preventDefault(); stopAutoTest(); }} 
                    ...
                    <button 
                      type="submit" 
                      disabled={!prompt.trim() || isAutoTesting} 
                      className="p-2 bg-[#e3e3e3] hover:bg-white disabled:bg-[#333537] disabled:text-gray-500 rounded-full text-black transition-colors"
                    >
