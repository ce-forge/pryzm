...
export default function SessionDirectory() {
  ...
  // Optimistic Insertion for new chats
  useEffect(() => {
  ...
  return (
    <>
      ...
      {folders.map(folder => (
        <div 
          key={folder.id} 
          className={`pt-1 pb-1 transition-all duration-200 rounded-lg border ${dragTarget === folder.id ? 'border-blue-500 bg-[#282a2c]/50' : 'border-transparent hover:border-[#4b4d4f] hover:bg-[#1a1b1c]/80 hover:shadow-sm'}`}
