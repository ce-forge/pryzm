...
function AssistantMessage({ content, searchQuery }: { content: string, searchQuery?: string }) {
  ...
  return (
    <ReactMarkdown
      ...
      components={{
        p({ children }) { return <p className="mb-4 last:mb-0 leading-relaxed min-w-0">{highlightChildren(children, searchQuery || "")}</p>; },
        blockquote({ children }) {
        ...
        code({ children }) {
          // FIX: Added whitespace-pre-wrap break-all to ensure inline code doesn't overflow
