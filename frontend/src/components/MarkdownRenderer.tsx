...
export default function MarkdownRenderer({ content, searchQuery, isActiveMatch }: { content: string, searchQuery?: string, isActiveMatch?: boolean }) {
  
  const highlightText = (text: string, query: string, isActive: boolean): React.ReactNode => {
  ...
  const highlightChildren = (children: React.ReactNode, query: string, isActive: boolean): React.ReactNode => {
    if (!query) return children;
    
    return React.Children.map(children, (child) => {
      if (typeof child === 'string') {
        return highlightText(child, query, isActive);
      }
      ...
      if (React.isValidElement(child)) {
        ...
        if (element.props?.children) {
           return React.cloneElement(element, {
              ...element.props,
              children: highlightChildren(element.props.children, query, isActive)
           });
        }
      }
    ...
    <ReactMarkdown
      ...
      components={{
        p({ children }) { 
          ...
          return <p className="mb-2 last:mb-0">{highlightChildren(children, searchQuery || "", !!isActiveMatch)}</p>; 
        },
        blockquote({ children }) {
          ...
          return (
            <blockquote className="bg-[#1a1b1c] border border-[#333537] border-l-4 border-l-blue-500 text-gray-300 px-4 py-3 rounded-r-lg shadow-md text-[13px] my-3 flex items-start gap-3">
              {Icon}
              <div className="flex-1 w-full">{highlightChildren(children, searchQuery || "", !!isActiveMatch)}</div>
            </blockquote>
          );
        },
        pre({ children }: any) { 
        ...
        th({ children }) { return <th className="px-4 py-2.5 font-semibold border-b border-[#333537]">{highlightChildren(children, searchQuery || "", !!isActiveMatch)}</th>; },
        td({ children }) { return <td className="px-4 py-2.5 text-gray-300">{highlightChildren(children, searchQuery || "", !!isActiveMatch)}</td>; },
        ...
        li({ children }) { return <li className="leading-snug">{highlightChildren(children, searchQuery || "", !!isActiveMatch)}</li>; },
      }}
    >
      {content}
    </ReactMarkdown>
